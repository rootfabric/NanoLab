...
Pure function of the repository's frozen evidence/config artifacts. A rerun
MUST be byte-identical (``--check`` compares the COMPLETE package, including
RELEASE_MANIFEST.json, which is generated here deterministically via
release.card_lint with a frozen release-metadata stamp — never wall-clock).

Scientific and protocol values (steps, seeds, temperature, salt, print
intervals, window, bootstrap parameters, engine commit, upstream pins) are
READ from evidence artifacts and cross-checked between independent sources;
any disagreement is a hard BuildError. Values transcribed by hand are not
allowed (Fresh Review R1 finding F-B1). The only code-owned constants are
release/contract metadata, explicitly classified below; none of them are
measured or run-derived numbers:

- ``VERSION`` — package semver (release metadata, contract §6/§7);
- ``RELEASE_GENERATED_AT_UTC`` / ``RELEASE_SUBJECT`` — frozen manifest
  assembly stamp/subject; regenerated byte-identically, NOT wall-clock
  (contract §7, repair R1 finding F-B2);
- ``ENGINE_DESCRIPTOR`` — engine identity string (naming, not a measurement);
- ``ANGLE_CONVENTION`` — verbatim frozen-convention citation
  (E2_OBSERVABLES_R2/erratum + frozen arm manifest);
- observable key names (``hinge_angle_confirmatory_200k`` etc.) — contract
...
Evidence inputs (repo-relative):
    ...
    docs/work/executions/EX-NL3-002-R1/evidence/c00{1,2,3}_input.in
    docs/work/executions/EX-NL3-002-PARAM-*/evidence/s00{1,2,3}_input.in
    docs/work/executions/EX-NL3-002-{R1,PARAM-11B,PARAM-32B,PARAM-53B}-R1/evidence/run-reports.json
    ...
    docs/research/REPRODUCTION_RULE_R1.md             (frozen rule, shipped verbatim)
...
from release import card_lint as card_lint_tool
...
CONFIRM_EXEC = REPO_ROOT / "docs/work/executions/EX-NL3-002-R1"
PARAM_SUMMARY = REPO_ROOT / "docs/work/executions/EX-NL3-002-SUMMARY-R1/evidence/parametric-summary.json"
CONFIRM_SUMMARY = CONFIRM_EXEC / "evidence/confirmatory-summary.json"
...
REPRODUCTION_RULE_DOC = REPO_ROOT / "docs/research/REPRODUCTION_RULE_R1.md"
...
# Run-config evidence (oxDNA input files of the published runs): the source of
# the protocol pins (steps/seed/T/salt/print intervals) for each campaign.
CONFIRM_RUN_INPUTS = [CONFIRM_EXEC / "evidence" / f"c00{index}_input.in" for index in (1, 2, 3)]
PARAM_RUN_INPUTS = {
    variant: [PARAM_EXEC[variant] / "evidence" / f"s00{index}_input.in" for index in (1, 2, 3)]
    for variant in ("11b", "32b", "53b", "74b")
}
# Machine-readable engine commit per executed campaign (74b has none: NOT_RUN).
RUN_REPORTS = {
    "0b": CONFIRM_EXEC / "evidence" / "run-reports.json",
    "11b": PARAM_EXEC["11b"] / "evidence" / "run-reports.json",
    "32b": PARAM_EXEC["32b"] / "evidence" / "run-reports.json",
    "53b": PARAM_EXEC["53b"] / "evidence" / "run-reports.json",
}
...
# --- release/contract metadata (code-owned; NOT evidence-derived science) ---
VERSION = "0.1.0-rc0"
RELEASE_GENERATED_AT_UTC = "2026-09-13T15:32:04Z"  # frozen at package assembly; byte-stable, never wall-clock
RELEASE_SUBJECT = {
    "repository": "rootfabric/NanoLab",
    "commit": "be73c5408bee54bc66105f0dee3e3262905eb2ad",  # assembly subject of the reviewed candidate
}
ENGINE_DESCRIPTOR = "oxDNA (CPU build, double precision)"
...
def _engine_platform() -> str:
    """Short OS/platform string from the frozen engine environment doc."""
    match = re.search(r"WSL2 Ubuntu [0-9.]+", ENGINE_ENV.read_text(encoding="utf-8"))
    if not match:
        raise BuildError(f"{ENGINE_ENV}: platform (WSL2 Ubuntu ...) not found")
    return match.group(0)


def _run_engine_commit(variant: str, engine: str) -> str:
    """Engine commit recorded by the campaign itself; cross-checked."""
    path = RUN_REPORTS.get(variant)
    if path is None:
        return engine
    commit = _read_json(path).get("engine_source_commit")
    if commit != engine:
        raise BuildError(f"{path}: engine_source_commit {commit!r} != engine pin {engine!r}")
    return commit


def _read_oxdna_input(path: Path) -> dict[str, str]:
    """Parse an oxDNA input file (frozen run config) into key -> text value."""
    values: dict[str, str] = {}
...
def _temperature_k(text: str) -> str:
    match = re.fullmatch(r"([0-9]+(?:\.[0-9]+)?)\s*K", text, re.IGNORECASE)
    if not match:
        raise BuildError(f"unsupported temperature format: {text!r}")
    number = float(match.group(1))
    return f"{int(number)} K" if number.is_integer() else f"{number} K"


def _int_field(values: dict[str, str], key: str, path: Path) -> int:
    try:
        return int(values[key])
    except (KeyError, ValueError) as exc:
        raise BuildError(f"{path}: field {key!r} is missing/not an integer") from exc


def _protocol_from_inputs(
    label: str,
    input_paths: list[Path],
    expected_seeds: list[int],
    shared: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Derive protocol pins from the run-config evidence files.

    Every pin is read from the published input files; replicas must agree on
    shared fields; ``expected_seeds`` (from the published summary) and, when
    given, the confirmatory environment record must match exactly.
    """
    if len(input_paths) != len(expected_seeds):
        raise BuildError(f"{label}: {len(input_paths)} input files vs {len(expected_seeds)} expected seeds")
    parsed = [(path, _read_oxdna_input(path)) for path in input_paths]
    seeds = sorted(_int_field(values, "seed", path) for path, values in parsed)
    if seeds != sorted(expected_seeds):
        raise BuildError(f"{label}: input seeds {seeds} != published seeds {sorted(expected_seeds)}")
    protocol: dict[str, Any] = {"seeds": seeds}
    for key, cast in (
        ("steps", int),
        ...
        for path, values in parsed:
            try:
                numbers.append(cast(values[key]))
            except (KeyError, ValueError) as exc:
                raise BuildError(f"{path}: field {key!r} is missing/malformed") from exc
        if len(set(numbers)) != 1:
            raise BuildError(f"{label}: {key} differs across replicas: {numbers}")
        protocol[key] = numbers[0]
    temperatures = {_temperature_k(values.get("T", "")) for _, values in parsed}
    if len(temperatures) != 1:
        raise BuildError(f"{label}: temperature differs across replicas: {sorted(temperatures)}")
    protocol["temperature"] = temperatures.pop()
    if shared is not None:
        for key, expected in (
            ("steps", shared.get("steps")),
            ("salt_concentration", shared.get("salt_concentration")),
            ("print_conf_interval", shared.get("print_conf_interval")),
            ("print_energy_every", shared.get("print_energy_every")),
            ("temperature", shared.get("temperature")),
        ):
            if expected is not None and protocol[key] != expected:
                raise BuildError(f"{label}: {key} {protocol[key]!r} != environment record {expected!r}")
    return protocol


def _confirm_environment(evidence_card: dict[str, Any], protocol: dict[str, Any]) -> dict[str, Any]:
    """Cross-check the run-config-derived pins against the 0b evidence card."""
    environment = evidence_card["environment"]
    steps_expected = int(environment["steps_confirmatory"])
    if protocol["steps"] != steps_expected:
        raise BuildError(f"0b: input steps {protocol['steps']} != environment steps_confirmatory {steps_expected}")
    for key in ("salt_concentration", "print_conf_interval", "print_energy_every"):
        if float(environment[key]) != float(protocol[key]):
            raise BuildError(f"0b: {key} {protocol[key]!r} != environment record {environment[key]!r}")
    match = re.match(r"([0-9]+(?:\.[0-9]+)?)\s*K\b", str(environment["temperature"]))
    if not match or float(match.group(1)) != float(protocol["temperature"].split()[0]):
        raise BuildError(f"0b: temperature {protocol['temperature']!r} != environment record {environment['temperature']!r}")
    return environment


def _shared_config_gate(pins: dict[str, Any], variant: str, verification: dict[str, Any]) -> None:
    """License reusing the confirmatory environment across the parametric
    series: every variant must pin byte-identical pro_CPU.in content."""
    pro_pin = pins["files"].get("MD_Hinges/pro_CPU.in")
    pro_run = verification["files"].get("MD_Hinges/pro_CPU.in") or {}
    if pro_pin is None or not pro_run.get("sha256"):
        raise BuildError(f"{variant}: pro_CPU.in sha256 pin missing for shared-config gate")
    if pro_pin["sha256"] != pro_run["sha256"]:
        raise BuildError(f"{variant}: pro_CPU.in sha256 differs from the content-verified 0b pin")
...
def _rule_step(observable_names: list[str]) -> str:
    return (
        "5. сравнение с карточкой по замороженному правилу REPRODUCTION_RULE_R1"
        " (docs/research/REPRODUCTION_RULE_R1.md; копия в пакете reproduction/):"
        " медиана пер-репличных медиан >= 3 валидных реплик в пределах [min, max]"
        " пер-репличных медиан карточки; pooled bootstrap CI95 исходной оценки —"
        " не полоса допуска; вердикты REPRODUCTION_MATCH / REPRODUCTION_MISMATCH /"
        " INCONCLUSIVE / TECHNICAL_FAILURE (исполнитель: reproduction/reproduce.py evaluate);"
        f" наблюдаемые: {', '.join(observable_names)}"
    )


RULE_TOLERANCE_POLICY = (
    "REPRODUCTION_RULE_R1: M_new (медиана пер-репличных медиан, >=3 валидных реплик)"
    " в пределах [min, max] пер-репличных медиан карточки; pooled bootstrap CI95 —"
    " не полоса допуска; гейты кадра §4 E2_PROTO_R1; недостаточно валидных реплик —"
    " INCONCLUSIVE; техотказ/отказ digest-гейта — TECHNICAL_FAILURE; не PASS при отсутствии проверки"
)
...
def _card_0b(
    ...
    return {
        ...
        "variant": "0b",
        "function": (
            f"Compliant DNA hinge: взаимный угол плеч под внешним воздействием; в v0.1 измерено"
            f" равновесное распределение угла (confirmatory {confirm_steps} + общее окно {window_steps})."
            " Управляемый привод — предмет E5, не этой карточки"
        ),
        "interfaces": {
        ...
        "operating_range": {
            "temperature": f"{protocol['temperature']} (author config verbatim; decision rule E2-SETUP-R1 §5)",
            "salt_concentration_M": protocol["salt_concentration"],
        ...
        "protocol_pins": {
            ...
            "options": {
                "print_conf_interval": protocol["print_conf_interval"],
                "print_energy_every": protocol["print_energy_every"],
                "salt_concentration": protocol["salt_concentration"],
            },
        ...
        "measured_observables": {
            ...
            "hinge_angle_common_window_150k": {
                "units": "deg",
                "convention": ANGLE_CONVENTION + f"; общий кросс-вариантный интервал {window_steps // 1000}k (addendum §8 item 3)",
                ...
                "distribution": {
                    ...
                    "note": f"secondary cross-variant window; primary confirmatory {confirm_steps} result NOT revised",
        ...
        "integrity": {
            ...
            "details": [
                f"long_bond_fraction <= 0.1078: max {evidence_card['stability']['long_bond_fraction_max']}",
                f"pairs_fraction_v2 >= 0.50: min {evidence_card['stability']['pairs_fraction_v2_min']}",
                f"displacement <= 20.0: max {evidence_card['stability']['displacement_max_max']}",
                f"energy |drift| total, max across replicas: {evidence_card['stability']['energy_max_abs_drift_total_max_across_replicas']}",
            ],
        ...
        "reproduction": {
            ...
            "steps": list(evidence_card["reproduction"]["steps"]) + [
                _rule_step(["hinge_angle_confirmatory_200k", "hinge_angle_common_window_150k"])
            ],
            "expected": {
                "hinge_angle_confirmatory_200k": pooled["median_deg"],
                "hinge_angle_common_window_150k": window["median_deg"],
            },
            "tolerance_policy": RULE_TOLERANCE_POLICY,
        ...
        "provenance": {
            ...
            "card_generated_from": [
                ...
                PARAM_SUMMARY_REL,
                "docs/work/executions/EX-NL3-002-SUMMARY-R1/evidence/component-card-0b.json",
                "docs/work/executions/EX-NL3-002-R1/evidence/c001_input.in",
                "docs/work/executions/EX-NL3-002-R1/evidence/run-reports.json",
                "scripts/hinge_family/source_pins.json",
                "docs/research/ENGINE_ENVIRONMENT_R1.md",
                "docs/research/REPRODUCTION_RULE_R1.md",
            ],
            "generation": "built by release.build_library.py (WO-NL5-001-B-R1 repair R1) from published evidence; protocol pins derived from run-config evidence and cross-checked; deterministic and byte-reproducible (--check, manifest included)",
        },
    }
...
def build(root: Path = PKG_ROOT) -> list[Path]:
    ...
    # Window: the common comparison window is evidence, not a constant.
    window_steps = int(param["window"]["common_window_steps"])
    for variant in VARIANTS:
        if int(param["variants"][variant]["common_window_steps"]) != window_steps:
            raise BuildError(f"{variant}: common_window_steps != summary window {window_steps}")

    # Engine pin is cross-checked against each executed campaign's own report.
    for variant in RUN_REPORTS:
        _run_engine_commit(variant, engine)

    # 0b protocol pins: derived from the confirmatory run-config evidence.
    confirm_seeds = [confirm["runs"][run_id]["seed"] for run_id in sorted(confirm["runs"])]
    protocol_0b = _protocol_from_inputs("0b", CONFIRM_RUN_INPUTS, confirm_seeds)

    # Parametric protocol pins: derived per variant; the shared author config
    # (pro_CPU.in, content-verified sha256) licenses reusing the 0b environment
    # record as the cross-check basis.
    protocols: dict[str, dict[str, Any]] = {"0b": protocol_0b}
    for variant in ("11b", "32b", "53b"):
        verification = _download_verification(variant)
        _shared_config_gate(pins, variant, verification)
        reps = param["variants"][variant]["per_replica_common_window"]
        seeds = [reps[key]["seed"] for key in sorted(reps)]
        protocols[variant] = _protocol_from_inputs(
            variant,
            PARAM_RUN_INPUTS[variant],
            seeds,
            shared={
                "salt_concentration": protocol_0b["salt_concentration"],
                "print_conf_interval": protocol_0b["print_conf_interval"],
                "print_energy_every": protocol_0b["print_energy_every"],
                "temperature": protocol_0b["temperature"],
            },
        )
        if protocols[variant]["steps"] < window_steps:
            raise BuildError(f"{variant}: run steps {protocols[variant]['steps']} < window {window_steps}")
    ...
    emit("RIGHTS.json", _rights(pins))
    emit("families/dna_hinge/family.json", _family(pins, protocol_0b["steps"], window_steps))

    cards = {"0b": _card_0b(param, confirm, evidence_card, pins, engine, protocol_0b, window_steps)}
    for variant in ("11b", "32b", "53b"):
        cards[variant] = _card_parametric(variant, param, pins, engine, protocols[variant], window_steps)
    cards["74b"] = _card_74b(param, pins, engine, window_steps)
    for variant in VARIANTS:
        emit(f"families/dna_hinge/cards/{variant}.card.json", cards[variant])

    emit("provenance/source-digests.json", _source_digests(pins))
    emit("protocols/README.md", _protocols_readme(engine, window_steps))
    emit("reports/family-report.md", _family_report(cards, protocol_0b["steps"], window_steps))
    ...
    # frozen reproduction rule travels with the package (byte copy)
    emit_bytes("reproduction/REPRODUCTION_RULE_R1.md", rel_path_bytes(REPRODUCTION_RULE_DOC))

    # deterministic manifest (repair R1, finding F-B2): generated here from the
    # frozen release metadata — included in the byte-for-byte check.
    manifest = card_lint_tool.manifest_create(
        root,
        generated_at_utc=RELEASE_GENERATED_AT_UTC,
        generated_by=f"release.build_library {VERSION}",
        subject=RELEASE_SUBJECT,
    )
    emit("RELEASE_MANIFEST.json", manifest)
...
def _rights(pins: dict[str, Any]) -> dict[str, Any]:
    upstream = pins["source"]
    return {
        "schema_version": 1,
...
def _family(pins: dict[str, Any], confirm_steps: int, window_steps: int) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "family_id": "dna_hinge",
        ...
        "measurement_basis": (
            f"0b: confirmatory {confirm_steps} (frozen E2_PROTO_R1) + common window {window_steps};"
            f" 11b/32b/53b: параметрическая серия, общее окно {window_steps} (addendum §8);"
            " 74b: runs NOT_RUN (honest gap)"
        ),
    }
...
def _card_parametric(
    ...
    verification = _download_verification(variant)
    environment = _read_json(CARD_0B_EVIDENCE)["environment"]
    reps = param["variants"][variant]["per_replica_common_window"]
    seeds = [reps[key]["seed"] for key in sorted(reps)]
    ...
    steps_note = (
        f"прогоны полностью внутри общего окна {window_steps} (cross-check: data fully inside window)"
        if fully_inside
        else f"прогон длиннее общего окна: last frame {windows[sorted(windows)[0]]['time_last_steps']} steps; сравнение — окно {window_steps} (addendum §8)"
    )
    bootstrap = pooled["bootstrap"]
    ...
    return {
        ...
        "family": "dna_hinge",
        "variant": variant,
        "function": f"Compliant DNA hinge: взаимный угол плеч; в v0.1 измерено равновесное распределение угла в общем сравнительном окне {window_steps} параметрической серии. Управляемый привод — предмет E5, не этой карточки",
        "interfaces": {
            "arms": f"два плеча (arm_a {manifest['arm_a']} / arm_b {manifest['arm_b']} нуклеотидов манифеста), топология {manifest['coverage']['topology_nucleotides']} нуклеотидов",
            "measurement_interface": ANGLE_CONVENTION,
        },
        ...
        "claim_ceiling": "C0_SOFTWARE_ONLY",
        "measurement_status": "MEASURED",
        "scientific_outcome": f"MEASURED (параметрическая серия, общее сравнительное окно {window_steps} steps; кросс-вариантные тренды в карточке не интерпретируются)",
        "protocol_pins": {
            "engine": ENGINE_DESCRIPTOR,
            "engine_commit": engine,
            "model": environment["model"],
            ...
            "steps": steps_value,
            "seeds": seeds,
            "platform": f"как в ENGINE_ENVIRONMENT_R1 ({platform})",
            "options": {
                "print_conf_interval": protocol["print_conf_interval"],
                "print_energy_every": protocol["print_energy_every"],
                "salt_concentration": protocol["salt_concentration"],
        ...
        "integrity": {
            ...
            "details": [
                "valid_frame_fraction_in_window = 1.0 в каждой реплике ("
                + ", ".join(
                    f"{key}: {windows[key]['frames_valid_in_window']}/{windows[key]['frames_in_window']}"
                    for key in sorted(frames_in_window)
                )
                + ")",
                f"кадры за пределами окна {window_steps} исключены frozen-правилом сравнения, а не гейт-отказом",
            ],
            "notes": "гейты §4 E2_PROTO_R1",
        },
        "known_gaps": [],
        "known_limitations": [
            f"измерение выполнено в общем сравнительном окне {window_steps} параметрической серии (addendum §8); confirmatory {protocol['steps']} в v0.1 есть только у варианта 0b",
            ...
            # family-level limitations, verbatim from the published E2 component
            # card (the remaining items there are 0b-run-specific)
            *_family_limitations(),
        ],
        "reproduction": {
            "requires": [
            ...
            "steps": [
                ...
                f"3. прогоны: CPU oxDNA, 3 реплики; анализ в общем окне {window_steps} steps (addendum §8)",
                f"4. frozen-анализ: v2 mutual-nearest + гейты §4 + статистика §6 (median/IQR/q05-q95 по валидным кадрам, bootstrap CI95 {bootstrap['resamples']} resamples, seed {bootstrap['seed']})",
                _rule_step(["hinge_angle_common_window_150k"]),
            ],
        ...
        "provenance": {
            ...
            "card_generated_from": [
                ...
                f"docs/work/executions/{PARAM_EXEC_ID[variant]}/evidence/s001_input.in",
                f"docs/work/executions/{PARAM_EXEC_ID[variant]}/evidence/run-reports.json",
                "docs/work/executions/EX-NL3-002-SUMMARY-R1/evidence/component-card-0b.json",
                manifest["source"],
                "scripts/hinge_family/source_pins.json",
                "docs/research/ENGINE_ENVIRONMENT_R1.md",
...
def _family_limitations() -> list[str]:
    """Family-level limitations, verbatim from the published E2 component card
    (the trailing items of that list are 0b-run-specific and stay on the 0b card)."""
    return list(_read_json(CARD_0B_EVIDENCE)["known_limitations"][:3])


def _card_74b(param: dict[str, Any], pins: dict[str, Any], engine: str, window_steps: int) -> dict[str, Any]:
    ...
    return {
        ...
        "protocol_pins": {
            "engine": ENGINE_DESCRIPTOR,
            "engine_commit": engine,
            "model": _read_json(CARD_0B_EVIDENCE)["environment"]["model"],
            "temperature": None,
            "steps": None,
            "seeds": [],
            "platform": f"как в ENGINE_ENVIRONMENT_R1 ({platform})",
        ...
        "provenance": {
            ...
            "generation": "built by release.build_library.py (WO-NL5-001-B-R1 repair R1) from published evidence; deterministic and byte-reproducible (--check, manifest included)",
        },
    }
...
def _protocols_readme(engine: str, window_steps: int) -> str:
...
{window_steps} steps) с observables `E2_OBSERVABLES_R2`.
...
Сравнение независимой кампании воспроизведения с карточками — только по
замороженному правилу `REPRODUCTION_RULE_R1` (копия: `reproduction/`).
"""


def _family_report(cards: dict[str, dict[str, Any]], confirm_steps: int, window_steps: int) -> str:
    rows = []
    for variant in VARIANTS:
        card = cards[variant]
        if card["measurement_status"] == "MEASURED":
            ...
            ci = obs["uncertainty"]
            basis = (
                f"confirmatory {confirm_steps} + window {window_steps}" if variant == "0b" else f"common window {window_steps}"
            )
            rows.append(f"| `{variant}` | {obs['estimate']:.9f} | [{ci[0]:.9f}, {ci[1]:.9f}] | {obs['n']} | {basis} |")
        else:
...
def check() -> list[str]:
    """Rebuild into a temp dir (manifest included) and compare byte-for-byte
    with the committed package: the COMPLETE package must be reproducible."""
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        committed = {p.relative_to(PKG_ROOT).as_posix() for p in PKG_ROOT.rglob("*") if p.is_file()}
