...
Схема **следует де-факто конвенции** campaign-level evidence-map, по которой опубликованы и приняты все три существующие карты: `E1-R1` (NL1-002), `E0-R4` (NL2-001), `E1-R2` (NL2-002). Прежняя checkpoint-видная форма схемы (обязательные `checkpoint`/`subject_sha`/`review_verdict`/`validation`/`artifacts`) не материализовалась ни в одном опубликованном документе и давала 11 идентичных ошибок на каждой кампанской карте (VERIFIER NL2-001 O3, NL2-002 F-1). Минимальный честный путь — «schema follow facts» (кампании приняты; формат — устоявшаяся конвенция).
