...
## evidence-map.schema.v1.json (sync NL2-003, F-1/O3)

Схема **следует де-факто конвенции** campaign-level evidence-map, по которой опубликованы и приняты все три существующие карты: `E1-R1` (NL1-002), `E0-R4` (NL2-001), `E1-R2` (NL2-002). Прежняя checkpoint-видная форма схемы (обязательные `checkpoint`/`subject_sha`/`review_verdict`/`validation`/`artifacts`) не материализовалась ни в одном опубликованном документе и отвергала все три кампанские карты — **11 / 11 / 9 ошибок** соответственно (E1-R1 / E0-R4 / E1-R2; число и состав не идентичны от карты к карте — erratum F-3 repair R1 NL2-003 к формулировке «11 идентичных»; VERIFIER NL2-001 O3, NL2-002 F-1). Минимальный честный путь — «schema follow facts» (кампании приняты; формат — устоявшаяся конвенция).

Ядро схемы (structural ceiling): `schema_version=1`, `kind=evidence_map`, identity/provenance-поля (`campaign_id`, `experiment_id`, `work_order_id`, `execution_id` = `EX-*`, `branch`, `base_sha` 40-hex, `risk_class`), `claim.campaign_scientific_outcome`+`campaign_ceiling`, `runs[]` (каждый run: `run_id` + `execution_outcome` из технического словаря), `failed_or_excluded_runs`, `remaining_risks`, `next_action`. Аналитические секции (`seed_distinctness`, `confirmatory_statistics`, `integrity`, `errata`, …) — кампанеспецифичны и сознательно открыты (`additionalProperties: true`): их семантику гейтит review, а не JSON-схема. Все три опубликованные карты валидны против схемы (0 ошибок; прогон NL2-003, jsonschema Draft 2020-12 + FormatChecker).
