...
Для experiment-событий применяется midnight-плейсхолд-проверка того же класса (`experiment_cli`; введена repair-коммитом R1 NL2-003 по REVIEWER F-1 — первоначальная публикация R1 декларировала этот контроль до реализации, что и было причиной вердикта FIX_REQUIRED); правило «≥3 копий» туда сознательно не перенесено: у быстрых прогонов терминал/анализ легитимно совпадают до секунды, а 3-событийные run'ы дали бы ложные срабатывания.
...
| midnight/непарсируемый/копия-штампы | `work_cli validate` (+ midnight в `experiment_cli`, repair R1) | O2/F3 |
