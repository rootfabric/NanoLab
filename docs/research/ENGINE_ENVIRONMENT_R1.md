...
Run ID: `EX-NL1-001-SMOKE-001`. Вход: копия fixture (хэши перепроверены на месте, 2/2), единственные отклонения от `quick_input`: `steps = 1e4` (вместо `1e6`) и `print_conf_interval = 1e4` (вместо `1e5`) — чтобы smoke не являлся protocol-прогоном E1 и дал по одной записи каждого вывода. Прогон выполнялся **до** принятия NL1-001 в соответствии с запретом `E1-PROTO-R1` §7 именно на verbatim-прогоны; изменённый smoke-вход под запрет §7 не подпадает и научной интерпретации не подлежит.
