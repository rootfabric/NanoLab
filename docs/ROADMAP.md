...
**Текущий frontier: `NL4` — AI NanoLab MVP; следующая работа `NL4-001` (ограниченный исследовательский агент; baselines — до E3). `NL0–NL3` приняты целиком; статусы экспериментов: `E0 = RUN`, `E1 = SUPPORTED`, `E2 = RUN` (0b измерен: 65.98° [65.67, 66.32]; параметрика 11b/32b/53b измерена; 74b — honest gap до arm-manifest-v2).** Актуальные статусы хранятся в [state.json](../project/state.json), зависимости работ — в [plan.json](../project/plan.json), сводка приёмок — в [WORK_QUEUE](work/WORK_QUEUE.md). Сроки и стоимость устанавливаются по измерениям, а не по обещанию ускорения от ИИ.
