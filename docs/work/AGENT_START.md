...
>
> Если frontier — `NL5`, следующая READY-задача — `NL5-001` «Создать первую библиотеку и release-пакет»: карточки проверенных компонентов (0b уже есть: 65.98° [65.67, 66.32]) в публиковable пакет; права derived results закрыты (G1 = B), LICENSE-решение — за владельцем. Измерительная база NL4: серия E2 + E3 (random/grid/LLM при равном бюджете; честный negative-on-advantage); 74b honest gap (arm-manifest-v2 отдельным WO). Не расширять научную линию сверх NL5-001 без решения владельца.
>
