...
>
> Если frontier — `NL5`, статус `NL5-002` — **WAITING_HUMAN**: external reproduction исполнено с terminal **MISMATCH** (2026-09-19, verified: 0b/32b вне frozen envelopes, 11b/53b MATCH; Fresh Reviewer PASS `be945f1`, Verifier PASS `4116468`; пороги не менялись). НЕ перезапускать кампанию, не «чинить» пороги, не стартовать NL6-001/E5 — решение за владельцем: `docs/evidence/NL5-002/DIRECTOR_DECISION_R1.md` (platform-sensitivity research WO / optional packaging v0.1.2 / NL5 acceptance policy). NL5-001 принят: пакет 0.1.0 опубликован, лицензии закрыты (D2); v0.1.1 — bounded repair (convention published, science byte-identical).
>
