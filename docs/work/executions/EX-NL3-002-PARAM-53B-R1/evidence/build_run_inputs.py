"""Build PARAM-53B run inputs (EX-NL3-002-PARAM-53B-R1) from the pinned author pro_CPU.in.

Per E2_PROTO_R1 §3 (inherited by WO-NL3-002-PARAM) + addendum §8, deviations
from pro_CPU.in verbatim are exactly: steps=150000 (addendum §8 parametric
window), per-run seed, output file names, print_conf_interval=4000,
print_energy_every=100, lastconf_file (absent in author input, added), plus the
subject change topology/conf_file 74b->53b. Every deviation is recorded in
PARAM-53B-S00X_deviations.json. All digest-gated inputs (53b.top, 53b.conf,
pro_CPU.in) and the built input are copied into each run directory (lesson F-1)
and byte-verified.
"""
import hashlib
import json
...
SRC = os.path.join(os.environ.get("TEMP", r"C:\Windows\Temp"), "nl3-002-param-53b-src")
SRC_FILES = {
    "MD_Hinges/53b.top": "53b.top",
    "MD_Hinges/53b.conf": "53b.conf",
...
STEPS = 150000  # E2_PROTO_R1 addendum section 8 (parametric window, 2026-09-12)
PRINT_CONF_INTERVAL = 4000
PRINT_ENERGY_EVERY = 100
REASON_STEPS = "E2_PROTO_R1 addendum section 8 (parametric series window 150k; base 2e7 never run)"
            ...
            if key in key_map:
                ...
                if line.split("=", 1)[1].strip() != value:
                    deviations.append({"key": key, "author": line.split("=", 1)[1].strip(),
                                       "run": value,
                                       "reason": REASON_STEPS if key == "steps" else REASON})
...
def main() -> int:
    ...
    report = {"schema_version": 1, "kind": "e2_param_input_build",
              "execution_id": "EX-NL3-002-PARAM-53B-R1", "variant": "53b",
              "protocol": "docs/research/E2_PROTO_R1.md (frozen, inherited, incl. addendum s.8)",
