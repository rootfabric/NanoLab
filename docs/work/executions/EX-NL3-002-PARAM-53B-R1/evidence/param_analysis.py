"""Parametric analysis for EX-NL3-002-PARAM-53B-R1 (E2_PROTO_R1 frozen, inherited, incl. addendum s.8).

Per replica: observables v2 (mutual) + integrity v1 + hinge angle with the
pre-run derived arm-manifest-53b.json (arm-manifest-v1, definition-before-data);
frame validity gates per protocol section 4 (lbf > 0.1078 / pf_v2 < 0.50 /
disp > 20.0); per-replica and pooled angle statistics over VALID frames
(median/IQR/q5/q95 + bootstrap CI, 10000 resamples, seed 424242); energy drift;
valid-frame fraction. All frames reported. scientific_outcome = MEASURED; no
interpretation (no cross-variant comparisons — Director after all batches).
"""
import argparse
import json
import math
...
REPO = r"C:\NanoLab\nl3-002-param-53b"
EX = os.path.join(REPO, "docs", "work", "executions", "EX-NL3-002-PARAM-53B-R1")
...
WSL_ROOT = "/home/yurig/nl3-002-param-53b/runs"
RUNS = [
    {"run_id": "PARAM-53B-S001", "prefix": "s001", "seed": 201004},
    {"run_id": "PARAM-53B-S002", "prefix": "s002", "seed": 202008},
    {"run_id": "PARAM-53B-S003", "prefix": "s003", "seed": 203012},
]
STEPS = 150000  # addendum section 8
FROZEN_MANIFEST = os.path.join(EX, "evidence", "arm-manifest-53b.json")
# E2_PROTO_R1 section 4 gates (frozen)
GATE_LBF_MAX = 0.1078
GATE_PF2_MIN = 0.50
GATE_DISP_MAX = 20.0
BOOT_N = 10000
...
def wsl_read(path, binary=False):
    # copy via \\wsl.localhost to Windows temp then read locally (trajectories ~60+ MB)
    tmp = os.path.join(os.environ.get("TEMP", r"C:\Windows\Temp"), "nl3-002-param-53b-read")
...
def analyse_run(run, manifest, topology_text):
    ...
    report = {
        "schema_version": 1,
        "kind": "e2_param_replica_analysis",
        "execution_id": "EX-NL3-002-PARAM-53B-R1",
        "protocol": "docs/research/E2_PROTO_R1.md (frozen, inherited, incl. addendum s.8: steps=150000) + docs/work/WO-NL3-002-PARAM.md",
        "variant": "53b",
        "run_id": rid, "seed": run["seed"], "steps_requested": STEPS,
        "detector": "v2 mutual-nearest (frozen, E2_OBSERVABLES_R2 + E2_PROTO_R1 §2)",
        "arm_manifest": "docs/work/executions/EX-NL3-002-PARAM-53B-R1/evidence/arm-manifest-53b.json (arm-manifest-v1, derived before runs; arm_a 4318 / arm_b 3638)",
        ...
        "scientific_outcome": "MEASURED (distributions; no interpretation; no cross-variant comparison in this execution; acceptance — Director after review)",
        "frames": frame_rows,
    }
    return report
...
def main() -> int:
    ...
    parser.add_argument("--only", help="comma-separated prefixes, e.g. s001")
    args = parser.parse_args()
    with open(FROZEN_MANIFEST, "r", encoding="utf-8", newline="") as h:
        manifest = json.load(h)["arms"]
    for name in ("arm_a", "arm_b"):
        if name not in manifest or "nucleotides" not in manifest[name]:
            raise ValueError(f"arm manifest missing {name}.nucleotides")
    topology_text = wsl_read(f"{WSL_ROOT}/PARAM-53B-S001/53b.top")
    ...
    if len(reports) == len(RUNS):
        ...
        with open(os.path.join(EX, "evidence", "run-reports.json"), "r", encoding="utf-8", newline="") as h:
            run_reports = json.load(h)
        manifest_full = json.load(open(FROZEN_MANIFEST, "r", encoding="utf-8", newline=""))
        summary = {
            "schema_version": 1,
            "kind": "e2_param_summary",
            "execution_id": "EX-NL3-002-PARAM-53B-R1",
            "variant": "53b",
            "steps": STEPS,
            "protocol": "docs/research/E2_PROTO_R1.md (frozen, inherited, incl. addendum s.8: steps=150000) + docs/work/WO-NL3-002-PARAM.md",
            "arm_manifest_53b": {"arm_a": manifest_full["arms"]["arm_a"]["size"],
                                 "arm_b": manifest_full["arms"]["arm_b"]["size"],
                                 "coverage_fraction_of_paired": manifest_full["coverage"]["manifest_fraction_of_paired"],
                                 "angle_frame0_deg": manifest_full["validation"]["hinge_angle_frame0"]["angle_deg"],
                                 "derivation": "arm-manifest-v1, byte-identical double run, definition-before-data"},
            "scientific_outcome": "MEASURED (measured distributions; no scientific conclusions; no cross-variant comparisons here; acceptance — Director after review)",
            "run_reports": run_reports["runs"],
            "runs": {r["run_id"]: {
        ...
        with open(os.path.join(EX, "evidence", "PARAM-53B-summary.json"), "w",
                  encoding="utf-8", newline="\n") as h:
