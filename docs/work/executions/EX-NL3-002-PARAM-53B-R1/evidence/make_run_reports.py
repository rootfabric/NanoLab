"""Collect run reports for EX-NL3-002-PARAM-53B-R1 from the per-run marker files.
...
def main() -> int:
    report = {"schema_version": 1, "kind": "e2_param_run_reports",
              ...
              "launch_method": "setsid+nohup detached bash per replica; exit codes and epochs written to files in run dirs (no wrapper dependency); detached budget watchdog SIGTERM at 12600 s -> ABORTED_BUDGET_INTERRUPT",
              "runs": {}}
    for run in RUNS:
        ...
        out = wsl(
            ...
            f"cat '{d}/end_epoch.txt' 2>/dev/null; cat '{d}/budget_interrupt.txt' 2>/dev/null; "
            f"wc -l < '{d}/{p}_energy.dat'; "
        ...
        if rest and rest[0].startswith("EXIT:"):
            exit_code = rest[0].split(":", 1)[1]
            rest = rest[1:]
        if rest and rest[0].isdigit():
            end_epoch = int(rest[0])
            rest = rest[1:]
        if rest and rest[0] == "ABORTED_BUDGET_INTERRUPT":
        ...
        report["runs"][rid] = {
            ...
            "interrupted_budget": budget_interrupt,
