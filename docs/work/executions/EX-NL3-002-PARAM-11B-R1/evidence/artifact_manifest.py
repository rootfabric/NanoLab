"""Artifact manifests for EX-NL3-002-PARAM-11B-R1 runs: SHA-256 + size of every
run input/output (WSL side, outside Git; trajectories ~130+ MB stay out of Git)."""
import json
import os
import subprocess
import sys
...
def wsl_stat(wsl_dir, name):
    ...
    return {"file": name, "role": "input" if name in ("11b.top", "11b.conf", "pro_CPU.in", "input.in")
...
def main() -> int:
    for run in RUNS:
        ...
        p = run["prefix"]
        files = ["11b.top", "11b.conf", "pro_CPU.in", "input.in",
                 f"{p}_traj.dat", f"{p}_energy.dat", f"{p}_last.dat", f"{p}_log.dat",
                 "engine_stdout.txt", "engine_stderr.txt"]
        # NOTE: no exit_code.txt — engines SIGTERM-killed at the 3.5h budget
        # deadline (pkill matched the parent bash too); see run-reports.json
        manifest = {
            "schema_version": 1,
