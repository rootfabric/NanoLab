"""Parametric analysis for EX-NL3-002-PARAM-11B-R1 (E2_PROTO_R1 frozen, inherited).

Per replica: observables v2 (mutual) + integrity v1 + hinge angle with the
pre-run derived arm-manifest-11b.json (arm-manifest-v1, definition-before-data);
...
FROZEN_MANIFEST = os.path.join(EX, "evidence", "arm-manifest-11b.json")
...
def wsl_read(path, binary=False):
    # copy via \\wsl.localhost to Windows temp then read locally (trajectories ~50 MB)
    tmp = os.path.join(os.environ.get("TEMP", r"C:\Windows\Temp"), "nl3-002-param-11b-read")
...
def analyse_run(run, manifest, topology_text):
    ...
    report = {
        ...
        "protocol": "docs/research/E2_PROTO_R1.md (frozen, inherited) + docs/work/WO-NL3-002-PARAM.md",
        "variant": "11b",
        "run_id": rid, "seed": run["seed"], "steps_requested": 200000,
        "detector": "v2 mutual-nearest (frozen, E2_OBSERVABLES_R2 + E2_PROTO_R1 §2)",
        "arm_manifest": "docs/work/executions/EX-NL3-002-PARAM-11B-R1/evidence/arm-manifest-11b.json (arm-manifest-v1, derived before runs; arm_a 4218 / arm_b 3722)",
...
def main() -> int:
    ...
    topology_text = wsl_read(f"{WSL_ROOT}/PARAM-11B-S001/11b.top")
    ...
    if len(reports) == len(RUNS):
        ...
        summary = {
            ...
            "variant": "11b",
            "protocol": "docs/research/E2_PROTO_R1.md (frozen, inherited) + docs/work/WO-NL3-002-PARAM.md",
            "arm_manifest_11b": {"arm_a": 4218, "arm_b": 3722,
                                 "coverage_fraction_of_paired": 1.0,
                                 "angle_frame0_deg": 74.357957026,
        ...
        with open(os.path.join(EX, "evidence", "PARAM-11B-summary.json"), "w",
                  encoding="utf-8", newline="\n") as h:
