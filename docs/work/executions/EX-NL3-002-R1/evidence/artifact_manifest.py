"""Artifact manifests for EX-NL3-002-R1 runs: SHA-256 + size of every run
input/output (WSL side, outside Git; trajectories ~130+ MB stay out of Git)."""
import json
import os
import subprocess
import sys
...
def wsl_stat(wsl_dir, name):
    ...
    return {"file": name, "role": "input" if name in ("0b.top", "0b.conf", "pro_CPU.in", "input.in")
...
def main() -> int:
    for run in RUNS:
        ...
        write_text(os.path.join("evidence", f"{p}-artifacts.manifest.json"),
