"""Build confirmatory run inputs (EX-NL3-002-R1) from the pinned author pro_CPU.in.

Per E2_PROTO_R1 §3, deviations from pro_CPU.in verbatim are exactly:
steps=200000, per-run seed, output file names, print_conf_interval=4000,
print_energy_every=100, lastconf_file (absent in author input, added), plus the
subject change topology/conf_file 74b->0b (the frozen subject of E2). Every
deviation is recorded in c00X_deviations.json. All digest-gated inputs
(0b.top, 0b.conf, pro_CPU.in) and the built input are copied into each run
directory (lesson F-1).
"""
import json
import os
import subprocess
...
SRC = os.path.join(os.environ.get("TEMP", r"C:\Windows\Temp"), "nl3-002-confirm-src")
SRC_FILES = {
    "MD_Hinges/0b.top": "0b.top",
    "MD_Hinges/0b.conf": "0b.conf",
...
REASON = "E2_PROTO_R1 frozen confirmatory protocol / per-run naming"
...
def build_input(src_text: str, run: dict):
    ...
    deviations.append({"key": "lastconf_file", "author": None,
                       "confirm": f"{run['prefix']}_last.dat",
...
def main() -> int:
    ...
    report = {"schema_version": 1, "kind": "e2_confirm_input_build",
    ...
    for run in RUNS:
        ...
        with open(os.path.join(EX, "evidence", f"{run['prefix']}_deviations.json"), "w",
                  encoding="utf-8", newline="\n") as h:
            h.write(canonical_json({"schema_version": 1,
                                    "kind": "e2_confirm_input_deviations",
                                    "run_id": run["run_id"],
        ...
        # verify copies byte-identical (sha256) to the digest-gated sources
        checks = {}
        for src_name, dest in list(SRC_FILES.items()) + [("INPUT", "input.in")]:
            if src_name == "INPUT":
                win_sha = wsl(f"sha256sum '{win_input.replace(chr(92), '/').replace('C:', '/mnt/c')}'").split()[0]
            else:
