...
- **Repair R1 (2026-09-10), задокументированное вынужденное отклонение (REVIEWER F-5):** `tests/test_work_cli_corrections.py` изменён (не создан), тогда как WO формулирует allowed paths как «tests/** (новые unittest-файлы)» (паспорт допускает `tests/**` без ограничения). Отклонение вынужденное: константный штамп генератора фикстур стал бы ошибкой нового валидатора O2/F3; семантика проверяемых правил не изменилась (только per-event штампы + комментарий; подтверждено REVIEWER'ом по diff). Принято к сведению как documented deviation.
- `e0_runner.py` включён в allowed_paths сознательно: REPAIR_MAP_F1_R1 §5.1 объявляет emit_run-фикс ОБЯЗАТЕЛЬНЫМ для будущих E0-tooling кампаний; зафиксировать его иначе (вне published-инструмента) невозможно. Исторические subject'ы кампаний сохраняют старые блобы в git-истории (provenance не нарушается).
- Repair R1 также затрагивает `scripts/harness/experiment_cli.py` (midnight-проверка, REVIEWER F-1 путь (a)) и документы erratum'ов — в пределах allowed_paths.
