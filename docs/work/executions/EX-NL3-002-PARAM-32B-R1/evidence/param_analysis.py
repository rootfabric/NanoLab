"""Parametric analysis for EX-NL3-002-PARAM-32B-R1 (E2_PROTO_R1 frozen, inherited, incl. addendum s.8).

Per replica: observables v2 (mutual) + integrity v1 + hinge angle with the
pre-run derived arm-manifest-32b.json (arm-manifest-v1, definition-before-data);
...
FROZEN_MANIFEST = os.path.join(EX, "evidence", "arm-manifest-32b.json")
...
def wsl_read(path, binary=False):
    ...
    tmp = os.path.join(os.environ.get("TEMP", r"C:\Windows\Temp"), "nl3-002-param-32b-read")
...
def analyse_run(run, manifest, topology_text):
    ...
    report = {
        ...
        "arm_manifest": "docs/work/executions/EX-NL3-002-PARAM-32B-R1/evidence/arm-manifest-32b.json (arm-manifest-v1, derived before runs; arm_a 4814 / arm_b 3190)",
...
def main() -> int:
    ...
    topology_text = wsl_read(f"{WSL_ROOT}/PARAM-32B-S001/32b.top")
    ...
    if len(reports) == len(RUNS):
        ...
        summary = {
            ...
            "steps": STEPS,
            "protocol": "docs/research/E2_PROTO_R1.md (frozen, inherited, incl. addendum s.8: steps=150000) + docs/work/WO-NL3-002-PARAM.md",
            "arm_manifest_32b": {"arm_a": 4814, "arm_b": 3190,
                                 "coverage_fraction_of_paired": 0.9995005,
                                 "angle_frame0_deg": 77.477102136,
        ...
        with open(os.path.join(EX, "evidence", "PARAM-32B-summary.json"), "w",
                  encoding="utf-8", newline="\n") as h:
